...
#     # Create a list to store the recommended courses in the desired format
#     recommended_courses_details = []
#     for i, course_id in enumerate(recommendations):
#         course_data = courses_df[courses_df['course_id'] == course_id].iloc[0]
#         course_description = course_data['description']
#         recommended_courses_details.append(f"({i+1}. {course_description} (Course ID: {course_id}))")
    
#     # Create or update the document for the user
...
def recommend():
    ...
    past_experience = request.json['pastExperience']
    skills = request.json['skills']
    
    # Update the user's interests in the users_df DataFrame
    users_df.loc[users_df['userId'] == user_id, 'interests'] = interests
    
    # Re-create the interaction matrix and re-train the models
    interaction_matrix = create_interaction_matrix(users_df, courses_df)
    ratings_df = interaction_matrix.stack().reset_index()
    ratings_df.columns = ['userId', 'course_id', 'rating']
    
    # Recreate dataset and re-train the models
    data = Dataset.load_from_df(ratings_df[['userId', 'course_id', 'rating']], reader)
    trainset, testset = train_test_split(data, test_size=0.2)
    algo.fit(trainset)
